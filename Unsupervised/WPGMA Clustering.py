...
class WPGMA_Linkage(HCSuper):
    def __init__(self,df):
        super().__init__(df)

    def Find_Min_Distance3(self,M,I,J,choice,minimum):
            if choice ==0:
                update=[]
                update2=[]
                update3=[]
                ...
                return (new_update[0]+new_update[1])/2   
        ...
        for i in range(W):
            if i<=0:
                ...
                if len(clusters_history)==2:
                    ...
                    minimum=HC.Find_Min_Distance3(M,I,J,1,minimum)
                    Z_new=HC.Z_matrix(clusters,clusters2,minimum)
                    Z=np.vstack([Z,Z_new])
                    return(Z)
                UPDATE=HC.Find_Min_Distance3(M,I,J,0,minimum)
...
dn=dendrogram(Z)
plt.title('Our WPGMA Linkage Dendrogram')
plt.show()
